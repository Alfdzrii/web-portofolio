...
const INFO = [
  {
    label: 'Email', value: 'alfadzrii@email.com',
    icon: <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="w-5 h-5 text-red-600"><path d="M4 4h16c1.1 0 2 .9 2 2v12c0 1.1-.9 2-2 2H4c-1.1 0-2-.9-2-2V6c0-1.1.9-2 2-2z"/><polyline points="22,6 12,13 2,6"/></svg>,
  },
  {
    label: 'GitHub', value: 'github.com/Alfdzrii',
  ...
  {
    label: 'LinkedIn', value: 'linkedin.com/in/alfadzrii',
...
export default function Contact({ dark }) {
  ...
  const handleSubmit = (e) => {
    ...
    window.location.href = `mailto:alfadzrii@email.com?subject=${encodeURIComponent(subject)}&body=${body}`
  }

  return (
    <section id="contact" className={`relative py-24 px-6 lg:px-10 ${dark ? 'bg-zinc-950' : 'bg-white'}`}>
          ...
          <motion.div className="lg:col-span-2 flex flex-col gap-8" {...fadeUp(0.1)}>
            ...
            {INFO.map(({ icon, label, value }) => (
              <div key={label} className="flex items-start gap-4">
                <div className={`flex-shrink-0 w-10 h-10 flex items-center justify-center border-2 ${dark ? 'border-zinc-700 bg-zinc-900' : 'border-zinc-200 bg-zinc-50'}`}>
                  {icon}
                </div>
                <div>
                  <p className={`text-[10px] font-black tracking-[0.2em] uppercase mb-0.5 ${dark ? 'text-zinc-500' : 'text-zinc-400'}`}>{label}</p>
                  <p className={`text-sm font-medium ${dark ? 'text-zinc-300' : 'text-zinc-700'}`}>{value}</p>
                </div>
              </div>
            ))}
            <div className={`flex items-center gap-2.5 border-l-4 border-red-600 pl-4 py-2 ${dark ? 'bg-zinc-900' : 'bg-zinc-50'}`}>
